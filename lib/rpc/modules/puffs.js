...
const { toBuffer } = require('puffscoinjs-util')
...
class Puffs {
  /**
   * Create puffs_* RPC module
   * @param {Node} Node to which the module binds
   */
  constructor (node) {
    const service = node.services.find(s => s.name === 'puffs')
    this._chain = service.chain
    const puffsProtocol = service.protocols.find(p => p.name === 'puffs')
    this.puffsVersion = Math.max.apply(Math, puffsProtocol.versions)
  ...
  /**
   * Returns the current puffscoin version
  ...
  protocolVersion (params, cb) {
    cb(null, `0x${this.puffsVersion.toString(16)}`)
  }
}

module.exports = Puffs
