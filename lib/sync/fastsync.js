...
const BN = require('puffscoinjs-util').BN
const { short } = require('../util')

/**
 * Implements an PUFFScoin fast sync synchronizer
...
class FastSynchronizer extends Synchronizer {
  ...
  best () {
    ...
    for (let peer of peers) {
      const td = peer.puffs.status.td
      if ((!best && td.gte(this.chain.blocks.td)) ||
          (best && best.puffs.status.td.lt(td))) {
        best = peer
      }
    }
    return best
   ...
   */
  async latest (peer) {
    const headers = await peer.puffs.getBlockHeaders({
      block: peer.puffs.status.bestHash, max: 1
    })
    return headers[0]
  }
