...
/**
 * Implements an puffs/62 based block fetcher
...
class BlockFetcher extends Fetcher {
  ...
  async request (job) {
    ...
    const headers = await peer.puffs.getBlockHeaders({ block: first, max: count })
    const bodies = await peer.puffs.getBlockBodies(headers.map(h => h.hash()))
   ...
   */
  peer (job) {
    return this.pool.idle(p => p.puffs)
  }
}
