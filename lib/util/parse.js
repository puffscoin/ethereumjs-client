...
const Account = require('puffscoinjs-account').default
...
async function parseGpuffsState (alloc) {
  const trie = new Trie()
  const promises = []
...
async function parseGpuffsHeader (json) {
  ...
  header.stateRoot = (await parseGpuffsState(json.alloc)).root
  return header
}

async function parseGpuffsParams (json) {
  const header = await parseGpuffsHeader(json)
  const params = {
    name: json.name,
